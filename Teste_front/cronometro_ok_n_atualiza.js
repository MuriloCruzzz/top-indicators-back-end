...
document.getElementById('btn-lancar-parada').addEventListener('click', function() {
    updateParadaLinha ();
            ...
            var tempo_formatado = converterMinutosParaHora(tempoParadaValue);
    
            var obsValue = document.getElementById('obs').value;
    
            if (obsValue.length < 13 && obsValue.trim() !== "") {
                alert("O campo de observação deve ter pelo menos 13 caracteres.");
                return;
            ...
            if(obsValue == ""){
                obsValue = "0 comentários";
            }// Limita a 10 caracteres

            var novoBotao = document.createElement('button');
            novoBotao.textContent = 'Motivo ' + dropdownValue + ' - Tempo: ' + tempo_formatado + ' - Obs: ' + obsValue;

            // Adicionando estilos diretamente na criação do botão

            novoBotao.style.textAlign = 'left';        
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            data.forEach(function(parada) {
                ...
                var obs = parada.observação;
                var obs_formatada = obs.substring(0, 10);
                
                if(obs_formatada == ""){
                    obs_formatada = "s/comentários";
                }
                // Cria um novo botão para cada parada
                var tempo = parada.tempo;
                ...
                // Retorna o tempo formatado
                var tempo_formatado = (minutos < 10 ? '0' + minutos : minutos) + ':' + (segundos < 10 ? '0' + segundos : segundos);

                
                var novoBotao = document.createElement('button');
                novoBotao.textContent = 'Motivo: ' + parada.tipo + ' - Tempo: ' + tempo_formatado + ' - Obs: ' + obs_formatada;
    ...
    $.ajax({ 
        ...
        success: function(data) {
            ...
            var labelTextoParada = document.getElementById('texto-parada');
             ...
             var tipo = data.tipo;
            var dataCriacao = data.Data_parada;

            if(tipo == null){
                return;
            }
            
            document.getElementById('btn_parada_tipo_alerta').textContent = tipo;
            ...
            // Iniciar o contador com base no tempo de criação retornado do banco de dados
            iniciarContadorComBaseNoTempo(dataCriacao);
        },
        error: function(xhr, textStatus, errorThrown) {
            console.log('Erro ao obter dados da linha:', errorThrown);
        }
...
// Função para iniciar o contador com base no tempo retornado do banco de dados
function iniciarContadorComBaseNoTempo(tempoCriacao) {
    // Converte a string de tempo de criação em um objeto Date
    var dataCriacao = new Date(tempoCriacao);
    
    // Inicia o contador para atualizar a cada segundo
    contador = setInterval(function() {
        // Calcula o tempo decorrido desde o tempo de criação até o momento atual
        var tempoDecorrido = Date.now() - dataCriacao.getTime();
        
        // Calcula as horas, minutos e segundos a partir do tempo decorrido
        var horas = Math.floor((tempoDecorrido / (1000 * 60 * 60)) % 24); // Calcula as horas
        var minutos = Math.floor((tempoDecorrido / (1000 * 60)) % 60); // Calcula os minutos
        var segundos = Math.floor((tempoDecorrido / 1000) % 60); // Calcula os segundos

        // Formata o tempo para exibir no formato HH:MM:SS
        ...
        labelTextoParada.textContent = tempoFormatado;
    }, 1000);
}
...
function finalizarParadaLinhaTopo (){
    ...
    $.ajax({ 
        ...
        dataType: 'json',
        contentType: 'application/json',
        data: JSON.stringify({ id_producao: id_producao, tempo : tempo }),
                                 // Convertendo para JSON
        success: function(data) {
            // Manipular os dados retornados, por exemplo, exibindo-os na página
            atualizarParadaLinha();
