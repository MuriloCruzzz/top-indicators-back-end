...
function atualizarTextoHeading(data) {
   ...
   $('#label-quantidade-demanda').text(data[0].total_demandado.toLocaleString('pt-BR'));
    ...
    $('#label-pph').text(data[0].media_PPH.toLocaleString('pt-BR'));
    
    $('#refugo_media_diaria').text(data[0].media_diaria_refugo.toLocaleString('pt-BR'));
    

    var media_ku = data[0].media_KU;
    
    $('#label-ku').text(calcularPorcentagem2(media_ku) + "%");
    
    var media_yeld = data[0].media_YIELD;
    
    $('#label_yeld').text(calcularPorcentagem2(media_yeld) + "%");
...
function calcularPorcentagem2(A) {
    // Verifica se A ou B é nulo ou NaN
    if (A === 0) {
...
function carregarDadosEAtualizarTextoHeading() {
    ...
    $.ajax({
        ...
        type: 'POST',
        dataType: 'json',
        data: JSON.stringify({ data_inicial: data_inicial, data_final: data_final }),
        success: function(data) {
